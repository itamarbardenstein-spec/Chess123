...
{
    /// Logic and UI management for the chess puzzle board
    public partial class PuzzleGrid : PuzzleGridModel
    {
        #region Public Methods
        /// Initializes an easy difficulty puzzle layout with hardcoded piece positions
        public override void InitEasyPuzzleGrid(Grid board)
        {
            ...
            // Fill board with empty cells initially
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)
                    BoardPieces[i, j] = new Pawn(i, j, false, null);

            // Manual setup of the "Easy" puzzle scenario
            ...
            FinalizeGridInitialization();
        }
        /// Initializes a medium difficulty puzzle layout
        public override void InitMediumPuzzleGrid(Grid board)
        {
            ...
            // Manual setup of the "Medium" puzzle scenario
        ...
        /// Initializes a hard difficulty puzzle layout
        public override void InitHardPuzzleGrid(Grid board)
        {
            ...
            // Manual setup of the "Hard" puzzle scenario
            BoardPieces[0, 0] = new Rook(0, 0, false, Strings.BlackRook);
        ...
        /// Executes a predefined automated counter-move based on the current puzzle difficulty
        public override void MakeOpponentMove(string difficulty)
        {
            // Simple logic to simulate opponent response in puzzles
        ...
        /// Displays visual indicators for all possible legal moves of a selected piece
        ...
        /// Removes all visual legal move indicators from the board
        public override void ClearDots()
        {
            // Filter and remove all Ellipse shapes from the grid children
        ...
        /// Highlights a specific square to provide a hint to the user
        ...
        /// Highlights both the start and end squares of the correct puzzle move
        ...
        /// Resets all board squares to their original checkered colors
        public override void ClearBoardHighLights()
        {
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)
                {
                    ...
                    uiPiece.BackgroundColor = (i + j) % 2 == 0
                        ? Color.FromArgb(Strings.BoardColorWhite)
                        : Color.FromArgb(Strings.BoardColorBlack);
                }
        }
        /// Updates the visual display when a piece moves to a new location
        ...
        /// Generates a new piece object while preserving relevant state data
        public override Piece CreatePiece(Piece original, int row, int col)
        {
            ...
            // Factory logic to clone pieces with their current movement history
        ...
        /// Finalizes UI setup and maps logic pieces to their UI elements
        private void FinalizeGridInitialization()
        {
            // Loop through all squares to set colors, events, and add to the grid
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)
                {
        ...
        /// Triggers the move logic when a board square is clicked
        protected override void OnButtonClicked(object? sender, EventArgs e)
        {
            ButtonClicked?.Invoke(this, (Piece)sender!);
        }
        /// Syncs the visual properties of a UI cell with its corresponding logic piece
        protected override void UpdateCellUI(int row, int col)
        {
            // Update image source and team color on the UI layer
